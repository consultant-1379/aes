...
//
//	********************************************************
#ifndef DDTADM_CAA_INC_AES_DDT_IMM_FILEPEER_OI_H_
#define DDTADM_CAA_INC_AES_DDT_IMM_FILEPEER_OI_H_
...
namespace imm
{
	 ...
	 *
	 * @brief
	 * Object Implementer of FilePeer MO.
	 *
	 *
	 * @sa Base_OI
	 */
	class FilePeer_OI : public Base_OI
	{
 	 public:
		///  Constructor.
		FilePeer_OI(const std::string &managedObjectClassName, const std::string &objectImplementerName);
		...
		//UTILITY
		static void adjustPathSlashes(std::string& path);
 	  ...
 	  private:
		...
		struct filePeer
		{
			operation::filePeerInfo info;

			bool completed;
			ImmAction action;

			filePeer() : info(), completed(false), action(Nothing) {};

			const char* getFilePeerName() const { return info.getName(); };
			const char* getFilePeerDN() const { return info.getFilePeerDN(); };
		...
		/**	@brief
		 *
		 *	This method responsible for port number handling.
		 *
		 *	@param filePeerData: file peer data.
		 *
		 *	@param oiHandle: the object implementer handle.
		 *
		 *	@param ccbId: the ID of the Configuration Change Bundle.
		 ...
		 *	@remarks Remarks
		 */
		ACS_CC_ReturnType portNumberHandling(filePeer& filePeerData, ACS_APGCC_OiHandle oiHandle, ACS_APGCC_CcbId ccbId);

		typedef std::multimap<ACS_APGCC_CcbId, filePeer> operationTable_t;

		/** @brief
		 *
		 *  Map of pending FilePeer Operations
		 *
		 */
		operationTable_t m_filePeerOperationTable;
	};

} /* namespace ddt_imm */

#endif /* DDTADM_CAA_INC_AES_DDT_IMM_FILEPEER_OI_H_ */
