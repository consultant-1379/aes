...
AES_DDT_TRACE_DEFINE(AES_DDT_Store_Checkpoint_State_WaitCommit)

namespace store {
namespace checkpoint {
namespace state {

	WaitCommit::WaitCommit(StoreBase* checkpointObj)
	: BaseInterface(checkpointObj, WAIT_COMMIT)
	{
	...
	int WaitCommit::handleStep()
	{
		AES_DDT_TRACE_FUNCTION;
		m_lastError = m_checkpoint->checkForCommit();
		return m_lastError;
	}

	bool WaitCommit::getNextStep(boost::shared_ptr<BaseInterface>& nextStep)
	{
		AES_DDT_TRACE_FUNCTION;
		bool result = true;
