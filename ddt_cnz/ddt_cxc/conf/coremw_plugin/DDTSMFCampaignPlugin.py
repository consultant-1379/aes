...
def createSMFCampaignPlugin():
   return DDTSMFCampaignPlugin()

class DDTSMFCampaignPlugin(SMFCampaignPlugin):
   # initialization method
   # self: variable represents the instance of the object itself
   def __init__(self):
      super(DDTSMFCampaignPlugin, self).__init__()
      self.MY_COMPONENT_UID = "aes.ddt"
      self._info = None
      self._utils = None
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
   def cliAtCampInit(self):
      ...
      ddt = self._info.getComponent("aes.ddt")
      for bundle in ddt.swBundles:
         bundleName = bundle[1] 
      result = []
      ...
      if (self._actionType == SMFConstants.CT_INSTALL or self._actionType == SMFConstants.CT_UPGRADE or self._actionType == SMFConstants.CT_MIGRATE):
         cli = os.path.join("$OSAFCAMPAIGNROOT", self.MY_COMPONENT_UID, "scripts", "ddt_helper.sh" )
