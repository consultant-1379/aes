...
void
TextTestResult::addError (Test *test, CppUnitException *e)
{
...
void
TextTestResult::addFailure (Test *test, CppUnitException *e)
{
	...
	if (testFailures () != 0)
		{
          ...
          CppUnitException* e = failure->thrownException ();
					stream << i << ") "
						<< "line: "
						<< (e ? estring (e->lineNumber ()) : "") << " " << (e ? e->fileName () : "")
						<< " " << "\"" << failure->thrownException ()->what () << "\"" << endl;
					i++;
        }
    }
}
...
TextTestResult::printHeader (ostream& stream)
{
	if (wasSuccessful ())
		...
		{
			cout << endl << "!!!FAILURES!!!" << endl
				<< "Test Results:"<< endl
				<< "Run:  " << runTests () << "   Failures: " << testFailures () << "   Errors: "
				<< testErrors () << endl;
		}
}
