...
void __CLASS_NAME__::close () {
	if (g_tra_logger_ptr) {
		g_tra_logger_ptr->Close();
		g_tra_logger_ptr->~ACS_TRA_Logging();
	}
	g_tra_logger_ptr = 0;
}
...
void __CLASS_NAME__::dump (uint8_t * buffer, size_t size, size_t dumping_size, size_t dumping_line_length) {
	...
	for (size_t i = 0; (i < size) && (i < dumping_size); ) {
		...
		if (g_tra_logger_ptr)	g_tra_logger_ptr->Write(output_buffer, LOG_LEVEL_TRACE);
	}
}
...
ACS_TRA_LogResult __CLASS_NAME__::vlogf (int sys_errno, int syslog_priority, ACS_TRA_LogLevel level,
	...
	char_count = ::vsnprintf(buffer, AES_AFP_ARRAY_SIZE(buffer), format, ap);

	if (sys_errno ^ ERRNO_DISABLED_MASK) { // Add system errno information to the log message
		char errno_buf[1024];
		::snprintf(buffer + char_count, AES_AFP_ARRAY_SIZE(buffer) - char_count, " [errno == %d, errno_text == '%s']",
				sys_errno, ::strerror_r(sys_errno, errno_buf, AES_AFP_ARRAY_SIZE(errno_buf)));
	}
...
ACS_TRA_LogResult __CLASS_NAME__::vlogf_source (int sys_errno, int syslog_priority,
	...
	char_count = ::snprintf(buffer, AES_AFP_ARRAY_SIZE(buffer), "{%s@@%d} ", source_func_name, source_line);

	char_count += ::vsnprintf(buffer + char_count, AES_AFP_ARRAY_SIZE(buffer) - char_count, format, ap);
