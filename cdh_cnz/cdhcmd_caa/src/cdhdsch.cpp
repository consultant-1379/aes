...
void print_message(AES_CDH_Result result)
{
    if (result.code() == AES_CDH_RC_INCUSAGE)
    {
        cout << result << endl;
        cout << "Usage: " << endl;
        cout << "cdhdsch -s secondarydestination | -b backupdestination"<< endl;
        cout << "        | primarydestination destinationset"<< endl;


        cout << "cdhdsch -x destinationset"<< endl;
    }
    else
    {
        cout << result << endl;
...
//-------------------------------------------------------------------------
//  Main program cdhdsch
//-------------------------------------------------------------------------
int main(int argc, char* argv[]) 
{
	AES_CDH_Result result;
    
    AP_InitProcess ("AES_CDH_Cdhdsch", AP_COMMAND); 
    AP_SetCleanupAndCrashRoutine("AES_CDH_Cdhdsch",NULL);

    if (argc <= 31)
    {
		result = AES_CDH_DestinationSet::changeAttr(argc, argv);
