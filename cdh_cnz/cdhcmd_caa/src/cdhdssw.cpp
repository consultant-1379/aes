...
// AUTHOR 
//  2002-11-29 by UAB/UKY/AU Ulf Gustafsson
...
void print_message(AES_CDH_Result result)
{
    if (result.code() == AES_CDH_RC_INCUSAGE)
    {
        cout << result << endl;
        cout << "Usage: " << endl;
        cout << "cdhdssw destination destinationset"<< endl;
    }
    else
    {
        cout << result << endl;
...
//-------------------------------------------------------------------------
//  Main program cdhdssw
//-------------------------------------------------------------------------
int main(int argc, char* argv[]) 
{
	AES_CDH_Result result;
    
    AP_InitProcess ("AES_CDH_Cdhdssw", AP_COMMAND); 
    AP_SetCleanupAndCrashRoutine("AES_CDH_Cdhdssw",NULL);

    if (argc <= 31)
    {
        result = AES_CDH_DestinationSet::changeActive(argc, argv);
