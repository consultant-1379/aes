...
AES_CDH_TRACE_DEFINE(AES_CDH_FileRespDestSetCmdHandler);

using namespace std;

//! Constructor
AES_CDH_FileRespDestSetCmdHandler::AES_CDH_FileRespDestSetCmdHandler(string className , string szimpName,ACE_Thread_Manager* threadManager_)
...
AES_CDH_FileRespDestSetCmdHandler::~AES_CDH_FileRespDestSetCmdHandler()
{
	if(m_oiHandler!=0)
	{
		delete m_oiHandler;
...
ACS_CC_ReturnType AES_CDH_FileRespDestSetCmdHandler::setObjImpl()
{
	...
	{ 
		...
		if ( errorCode == ACS_CC_FAILURE )
		{
			...
			if ( i >= 10 )
			{
				AES_CDH_LOG(LOG_LEVEL_WARN, "Set implementer for the class %s : failed, Errorcode = %d",m_className.c_str(), intErr);
...
ACE_INT32 AES_CDH_FileRespDestSetCmdHandler::svc()
{
	AES_CDH_TRACE_MESSAGE("In svc Dispatch");

	m_poReactor->open(1);
	dispatch(m_poReactor, ACS_APGCC_DISPATCH_ALL);
	m_poReactor->run_reactor_event_loop();
	AES_CDH_TRACE_MESSAGE("Exiting svc Dispatch");
	return 0;
}


ACS_CC_ReturnType AES_CDH_FileRespDestSetCmdHandler::create(ACS_APGCC_OiHandle oiHandle, ACS_APGCC_CcbId ccbId, const char *className, const char* parentName, ACS_APGCC_AttrValues **attr)
{
	...
	{
		string tmpString;
		if(ACE_OS::strcmp(attr[i]->attrName, "respondingDestinationSetId")==0)
		{
			...
			if (isValidName(destSetName) == false)
			{
				...
				//set error code for invalid values
...
ACS_CC_ReturnType AES_CDH_FileRespDestSetCmdHandler::deleted(ACS_APGCC_OiHandle oiHandle, ACS_APGCC_CcbId ccbId, const char *objName)
{
	//Acquire mutex.
	ServR::respdestsetMutex.acquire();

	ServR::isRespDestSetDeleteTrig = true;
	ServR::isDelOfRespDestSetSuccess = false;
	...
	string destSetName(""), detailInfo(""), destName(""), userName(""), userGroup("");
	...
	if( !destSetAttrs[0].primDest.empty() )
	{
		...
		{
			AES_CDH_LOG(LOG_LEVEL_TRACE, "after push back");
			AES_CDH_LOG(LOG_LEVEL_DEBUG, "%s",m_deleteDestList[ctr].destinationSetName.c_str());
		}
	...
	if( !destSetAttrs[0].secDest.empty() )
	{
		...
		if ( resultCode != AES_CDH_RC_OK )
		{
			AES_CDH_LOG(LOG_LEVEL_INFO, " Error occured while removing dest %s from destset %s", destSetAttrs[0].secDest.c_str(), destSetName.c_str());
		...
		if ( resultCode != AES_CDH_RC_OK )
		{
			AES_CDH_LOG(LOG_LEVEL_INFO, " Error occured while removing secondary dest %s from destset %s", destName.c_str(), destSetName.c_str());
			AES_CDH_LOG(LOG_LEVEL_TRACE, " Leaving  with error code %d", resultCode);
			...
			ServR::respdestsetMutex.release();
			//set error code for result of deleteDest
			setErrorCode(resultCode);
			return ACS_CC_FAILURE;
		}
	}
	...
	if (resultCode != AES_CDH_RC_OK)
	{
		AES_CDH_LOG(LOG_LEVEL_INFO, "ServR::defineDestSet , failed");
		ServR::respdestsetMutex.release();
		//set error code for result of define
		setErrorCode(resultCode);
		return ACS_CC_FAILURE;
	}
	...
	if( !destSetAttrs[0].primDest.empty() )
	{
		...
		for ( unsigned ctr = 0 ; ctr < m_deleteDestList.size() ; ctr++)
		{
			AES_CDH_TRACE_MESSAGE("after delete success");
			AES_CDH_TRACE_MESSAGE("%s", m_deleteDestList[ctr].destinationSetName.c_str());
		}
	...
	ServR::isDelOfRespDestSetSuccess = true;
	ServR::respdestsetMutex.release();
...
ACS_CC_ReturnType AES_CDH_FileRespDestSetCmdHandler::modify(ACS_APGCC_OiHandle oiHandle, ACS_APGCC_CcbId ccbId, const char *objName, ACS_APGCC_AttrModification **attrMods)
{
	(void)attrMods;
	(void)oiHandle;
	(void)ccbId;
...
ACS_CC_ReturnType AES_CDH_FileRespDestSetCmdHandler::complete(ACS_APGCC_OiHandle oiHandle, ACS_APGCC_CcbId ccbId)
{
	(void)oiHandle;
	(void)ccbId;
...
void AES_CDH_FileRespDestSetCmdHandler::abort(ACS_APGCC_OiHandle oiHandle, ACS_APGCC_CcbId ccbId)
{
	...
	for ( unsigned ctr = 0 ; ctr < m_deleteDestList.size() ; ctr++)
	{
		AES_CDH_TRACE_MESSAGE("before abort");
		AES_CDH_TRACE_MESSAGE("%s", m_deleteDestList[ctr].destinationSetName.c_str());
	}
		...
		if(  m_deleteDestList.size() > 0 )
		{
			for ( unsigned ctr = 0 ; ctr < m_deleteDestList.size() ; ctr++)
			{
				if( m_deleteDestList[ctr].isPrimDest == true )
				{
					...
					ServR::define( m_deleteDestList[ctr].destinationName, m_deleteDestList[ctr].transferType, m_deleteDestList[ctr].argc, m_deleteDestList[ctr].argv, strUserGroup, AES_CDH_PRIM_RESP_DESTINATION_ID, m_deleteDestList[ctr].destinationSetName);
					ServR::defineDestSet(m_deleteDestList[ctr].destinationSetName, m_deleteDestList[ctr].destinationName, secDest, backupDest,strUserGroup, AES_CDH_RESP_DEST_SET_ID);
				}
			...
			for ( unsigned ctr = 0 ; ctr < m_deleteDestList.size() ; ctr++)
			{
				AES_CDH_TRACE_MESSAGE("before entire list  delete");
				AES_CDH_TRACE_MESSAGE("%s", m_deleteDestList[ctr].destinationSetName.c_str());
			}
			...
			for ( unsigned ctr = 0 ; ctr < m_deleteDestList.size() ; ctr++)
			{
				AES_CDH_TRACE_MESSAGE("after entire list delete");
				AES_CDH_TRACE_MESSAGE("%s", m_deleteDestList[ctr].destinationSetName.c_str());
			}

		}

		ServR::isRespDestSetDeleteTrig = false;
		ServR::respdestsetMutex.release();
	}
}

void AES_CDH_FileRespDestSetCmdHandler::apply(ACS_APGCC_OiHandle oiHandle, ACS_APGCC_CcbId ccbId)
{
	(void)oiHandle;
	AES_CDH_TRACE_MESSAGE("CcbApplyCallback invoked");
...
ACS_CC_ReturnType AES_CDH_FileRespDestSetCmdHandler::updateRuntime(const char *objName, const char **attrName)
{
	AES_CDH_TRACE_MESSAGE("CcbUpdateRuntimeCallback invoked");
	...
	parToModify.attrName = (char *) AES_CDH_RESP_ACTIVE_DEST_DESTSET;
	...
	{
		parToModify.attrValues[0] =reinterpret_cast<void*>(const_cast<char *>( activeDestName.c_str())); // pMyActiveDest;
...
void AES_CDH_FileRespDestSetCmdHandler::adminOperationCallback(ACS_APGCC_OiHandle oiHandle,
...
{
	(void)paramList;
	AES_CDH_TRACE_MESSAGE("adminOperationCallback invoked");
	...
	case 8:
	{
		...
		else
		{
			...
			AES_CDH_LOG(LOG_LEVEL_INFO, "In Admin call back , route traffic  failed ");
...
void AES_CDH_FileRespDestSetCmdHandler::shutdown()
{
	AES_CDH_TRACE_MESSAGE("Entering");
	...
	if( m_isClassImplAdded == true )
	{
		for(int i=0; i < 10; ++i)
		{
			if( m_oiHandler->removeClassImpl(this, m_className.c_str()) == ACS_CC_FAILURE )
			{
				...
				if ( intErr == -6 )
				{
					AES_CDH_LOG(LOG_LEVEL_WARN, "Error occurred while removing class implementer");
					for(int j=0; j< 100; ++j) ; //do nothing loop to wait for sometime, better than sleeping
					if ( i >= 10 )
					{
						AES_CDH_LOG(LOG_LEVEL_ERROR, "Error occurred while removing class implementer,ErrorCode = %d", intErr);
						break;
					}
					else
						continue;
...
=================================================================== */
void AES_CDH_FileRespDestSetCmdHandler::extractNameFromRdn( const string rdnName,
		string &name)
{
...
void AES_CDH_FileRespDestSetCmdHandler::extractNameFromDn (const string dnName, string& name)
{
	AES_CDH_TRACE_MESSAGE("Entering");
...
bool AES_CDH_FileRespDestSetCmdHandler::isValidName(string name)
{
...
=================================================================== */
void AES_CDH_FileRespDestSetCmdHandler::setErrorCode(unsigned int code, const char * objName)
{
