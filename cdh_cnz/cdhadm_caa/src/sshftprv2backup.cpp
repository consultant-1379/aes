/*******************************************************************************/
/**
@file sshftprv2backup.cpp

Class method implementation for sshftprv2backup.h

DESCRIPTION
The services provided by SSHFTPRV2Backup facilitates the SSH FTPv2 backup in responding mode..
...
GCC_TDEF(AES_CDH_SSHFTPRV2Backup);

/*===================================================================
   ROUTINE:SSHFTPRV2Backup
=================================================================== */
SSHFTPRV2Backup::SSHFTPRV2Backup() : backupWriter(0), apiClosed(false), oldRecordNumber(APPLID_MAX+1),switchState(AGENT_OK),isOpen(false),badFTPArea(false), bCease(true),TQrecv(0),bpMapper(0)
{
}
...
SSHFTPRV2Backup::~SSHFTPRV2Backup()
{
	if( backupWriter != 0 )
	{
		delete backupWriter;
...
AES_CDH_ResultCode SSHFTPRV2Backup::open(const string &streamName,
                                      const string &streamId,
                                      string &sourceId)
{
    GCC_TTRACE((AES_CDH_SSHFTPRV2Backup,
        "(%t) SSHFTPRV2Backup::open()\n"));
    ...
    if (backupWriter->open(destinationPtr->getDestSetName(), sourceId, this))
    {
        ...
        if (! bpMapper->open(sourceId, RPCName))
        {
            ...
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
        ...
        if (oldRecordNumber == APPLID_MAX + 1)
        {
            ...
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                "(%t) SSHFTPRV2Backup::open(), oldRecordNumber set to %u\n", oldRecordNumber));
        }
        else
        {
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                "(%t) SSHFTPRV2Backup::open(), oldRecordNumber == %u\n", oldRecordNumber));
        }
...
AES_CDH_ResultCode SSHFTPRV2Backup::close()
{
		
    GCC_TTRACE((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::close()\n"));
    transMutex.acquire();
    ...
    backupWriter->close();
    bpMapper->close(); 
    BlockPairMapperManager::instance()->remove(destinationPtr->getDestSetName());
    bpMapper=NULL;
    transMutex.release();
...
AES_CDH_ResultCode SSHFTPRV2Backup::writeToFile(AES_DBO_DataBlock *block)
{
	bool skipBlock = false;
    AES_CDH_ResultCode returnCode = AES_CDH_RC_OK;
    
    GCC_DEBUG(("(%t) FTPRV2Backup::SSHsendRecordFile() block %u received from the stream\n", block->blockNr_));
    
    if (block->blockNr_ != oldRecordNumber + 1)
    {
		GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup, "recordNumber != oldRecordNumber + 1, recordNumber = %d, oldRecordNumber + 1 = %d", block->blockNr_,(oldRecordNumber + 1)));
        if (block->blockNr_ <= bpMapper->getApplBlockId())
        {
            ...
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,"(%t) SSHFTPRV2Backup::eendRecordFil () calling setNextApplBlockId(..), CDRBid=%u\n", CDRBid));
        }
        else
        {
            // This block is not "wanted", just throw away block and get next block
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,"\nblock skipped!, block number = %d ", block->blockNr_));
            skipBlock = true;
        }
    }
    ...
    else
    {
        ...
        if (b != true)
        {
			GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,"\nSend Error, block number = %d", block->blockNr_));
...
AES_CDH_ResultCode SSHFTPRV2Backup::sendRecordFile(const string& streamName,
                                                const string& streamId,
                                                AES_DBO_DataBlock*& block,
                                                CmdHandler *cmdHdlr,
                                                const ACE_UINT64 ticks)
{
	GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::sendRecordFile() - entering\n"));
   ...
   if (GCC_rc == AES_BUFFERERROR)
   {
      ...
      GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                 "(%t) SSHFTPRV2Backup::sendRecordFile()\nError opening stream: %u\n"
   ...
   if (returnCode == AES_CDH_RC_OK)
   {
      while (!apiClosed)
      {
         ...
         {
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                        "(%t) SSHFTPRV2Backup::sendRecordFile(), block != NULL!\n"));
         ...
         else
         {
            ...
            case AES_NOSERVERACCESS:
               GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                           "(%t) SSHFTPRV2Backup::sendRecordFile() stream returned AES_NOSERVERACCESS\n"));
               break;

            case AES_BUFFERERROR:
               GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                           "(%t) SSHFTPRV2Backup::sendRecordFile() stream returned AES_BUFFERERROR\n"));
               returnCode = AES_CDH_RC_STREAMERROR;
               break;

            case AES_TIMEOUT:
               GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                           "(%t) SSHFTPRV2Backup::sendRecordFile() stream returned AES_TIMEOUT\n"));
            ...
            case AES_BUFFEREMPTY:
               GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                           "(%t) SSHFTPRV2Backup::sendRecordFile() stream returned AES_BUFFEREMPTY\n"));
               break;

            default:
               GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                           "(%t) SSHFTPRV2Backup::sendRecordFile() Unknown return code: %u\n",
                           GCC_rc));
               break;
            }
   ...
   {
      GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
                  "(%t) SSHFTPRV2Backup::sendRecordFile() open() error, returning AES_CDH_RC_SENDERR !!!\n"));
      returnCode = AES_CDH_RC_SENDERR;
   }
...
AES_CDH_ResultCode SSHFTPRV2Backup::transactionBegin()
{
    GCC_TTRACE((AES_CDH_SSHFTPRV2Backup,
        "(%t) SSHFTPRV2Backup::transactionBegin()\n"));
    return AES_CDH_RC_OK;
}
...
AES_CDH_ResultCode SSHFTPRV2Backup::transactionEnd(ACE_UINT32 &translatedBlockNr)
{
    ...
    {
        GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
            "(%t) SSHFTPRV2Backup::transactionEnd()\n"
...
AES_CDH_ResultCode SSHFTPRV2Backup::transactionCommit(ACE_UINT32 &applBlockNr)
{
	(void)applBlockNr;
#if 0
    AES_CDH_ResultCode rCode = AES_CDH_RC_OK;
    ...
    {
        GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
            "(%t) SSHFTPRV2Backup::transactionCommit()\n"
            "Destination is not open!\n"
            "blocknumber == %u\n", applBlockNr));
    ...
    if (rCode == AES_CDH_RC_OK)
    {
        ...
        {
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::transactionCommit()\n"
                "id.applid = %u\n"
        ...
        GCC_DEBUG(("(%t) SSHFTPRV2Backup::transactionCommit(), CDRBid=%u\n", CDRBid));
        GCC_DEBUG(("(%t) SSHFTPRV2Backup::transactionCommit(), applBlockNr=%u\n", applBlockNr));
    ...
    {
        GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::transactionCommit(), lastTransaction failed\n"));
    }
...
/*===================================================================
   ROUTINE:sendEvent
=================================================================== */
//  Added for trouble report HD83326
//******************************************************************************
void SSHFTPRV2Backup::sendEvent(CmdHandler *cmdHdlr, const ACE_UINT64 ticks)
{
...
AES_CDH_ResultCode SSHFTPRV2Backup::getLastCommittedBlock(ACE_UINT32 &applBlockNr)
{
	(void)applBlockNr;
    AES_CDH_ResultCode rCode = AES_CDH_RC_OK;
	ACE_UINT32 CDRBid = 0;
    ...
    {
        GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup,
            "(%t) SSHFTPRV2Backup::getLastCommittedBlock()\n"
            "Destination is not open!\n"));
...
AES_CDH_ResultCode SSHFTPRV2Backup::transactionTerminate()
{
    GCC_TTRACE((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::transactionTerminate()\n"));
    return AES_CDH_RC_OK;
}
...
TransferAgent::TransferAgentAttributes SSHFTPRV2Backup::getAgentType()
{
...
AES_CDH_ResultCode SSHFTPRV2Backup::define(ACE_INT32 &argc,
                                        ACE_TCHAR *argv[],
                                        const bool define,
                                        const bool recovery)
{
    ...
    if ((backupWriter = new BackupWriter()) == 0)
    {
        ...
        GCC_TFATAL((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::define()\n",
            "Error allocating BackupWriter!\n"));

        return AES_CDH_RC_INTPROGERR;
    }
	
    return SSHFTPRV2::define(argc, argv, define, recovery);
}
...
AES_CDH_ResultCode SSHFTPRV2Backup::setAPIClosed()
{
    if (this->isOpen)
        apiClosed = true;
    GCC_TTRACE((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::setAPIClosed()\n"));
    return AES_CDH_RC_OK;
}
...
void SSHFTPRV2Backup::switchAgent(bool stopTransfer)
{
    transMutex.acquire();
    // Transfer is in progress, order a stop after commit
    if (stopTransfer)
    {
        GCC_TINFO((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::switchAgent()"
    ...
    else
    {
    	...
    	GCC_DEBUG(("(%t) SSHFTPRV2Backup::switchAgent(), stopTransfer is false\n"));
        bpMapper = BlockPairMapperManager::instance()->getMapper(destinationPtr->getDestSetName());
        if (bpMapper != NULL)
        {
            ...
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::switchAgent()"
...
AES_CDH_ResultCode SSHFTPRV2Backup::remove()
{
    GCC_DEBUG(("(%t) SSHFTPRV2Backup::remove()\n"));
    return SSHFTPRV2::remove();
}
...
const string SSHFTPRV2Backup::getDestSetName()
{
...
AES_CDH_ResultCode SSHFTPRV2Backup::publishBackupFile()
{
    transMutex.acquire();
    ostringstream remoteFileName;
    remoteFileName << ::time(NULL);
    string transtype = "sshftprv2";
    ...
    if (path != "")
    {
        ...
        for (ACE_UINT32 i = 0; i < nFiles; i++)
        {
            ...
            GCC_TDEBUG((AES_CDH_SSHFTPRV2Backup, "(%t) SSHFTPRV2Backup::publishBackupFile()"
