...
void
TestRunner::printBanner ()
{
	cout << "Usage: testdriver [ -wait ] testName, where name is the name of a test case class" << endl;
}

void
TestRunner::run (int ac, char **av)
{
	string testCase;
  int numberOfTests = 0;
...
main (int ac, char **av)
{
	...
	runner.addTest("cdhdef", CdhdefTestCase::suite() );
    runner.addTest("cdhch", CdhchTestCase::suite() );
    runner.addTest("cdhsf", CdhsfTestCase::suite() );
	runner.addTest("cdhdsls", CdhdslsTestCase::suite() );
	runner.addTest("cdhdsdef", CdhdsdefTestCase::suite() );
	runner.addTest("cdhdsrm", CdhdsrmTestCase::suite() );
	runner.addTest("cdhdsch", CdhdschTestCase::suite() );
	runner.addTest("cdhdssw", CdhdsswTestCase::suite() );
    runner.run (ac, av);
